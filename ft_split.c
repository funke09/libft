...
/*   Updated: 2021/11/14 16:34:12 by zcherrad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cntchar(const char *s, int *start, char c)
{
	int	i;
	int	j;
...
char	**loop(t_vars vars, const char *s, char c)
{
	while (vars.i < vars.count)
	{
		vars.k = 0;
		vars.rsv[vars.i] = \
			malloc(sizeof(char) * (cntchar(s, &vars.start, c) + 1));
		if (!vars.rsv[vars.i])
		{
			while (vars.i-- >= 0)
...
char	**ft_split(char const *s, char c)
{
	...
	vars.rsv = malloc(sizeof (char *) * (vars.count + 1));
	if (!vars.rsv)
		return (NULL);
	if (!*s)
	{
